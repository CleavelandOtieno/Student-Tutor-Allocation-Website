...
foreach ($result as $row) {
	...
	if ($userActivity > $current_time) {
		$status = '<span class="label label-success">FULLY ONLINE</span>';
	}else{
	...
	$output .='
		...
		<td>'.$row['tutorName'].'</td>
		<td>'.$status.'</td>
		<td><button type="button" class="btn btn-info btn-xs start_chat" data-tousername="'.$row['tutorName'].'">Start Chat</button></td>
 </tr>
	';
}
$output .= '</table>';
